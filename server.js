...
var commonUtils = require('./utilities.js');
...
router.get('/contact', function (req, res) {
    ...
    let id = req.query.id
    if (id) {
        sql.query(`SELECT * FROM contact WHERE id=${id}`, function (err, result, fields) {
...
router.delete('/contestant', (req,res) => {
    ...
    let isBlank = commonUtils.isBlank('poopdi', recordId, res);
    let isInteger = commonUtils.integerValueIsANumber('Poooh', recordId, res);

    if(!isBlank) {
        if (isInteger) {
        let mysqlDelete = `DELETE FROM contestant WHERE id = ${recordId}`;
        sql.query(mysqlDelete, (error, result) => {
            if(error){
        ...
        //has been transformed to int but still broke
    } 
}else {
    console.log("is blank mothafucka");
